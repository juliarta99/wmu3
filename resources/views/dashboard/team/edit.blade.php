...
@section('content')
    <div class="flex items-center gap-3">
        ...
        <span class="text-xl md:text-2xl font-bold">Edit <span class="text-five">Team</span></span>
    </div>

    <form id="team-form" action="{{ route('dashboard.team.update', $team->id) }}" method="POST" class="flex flex-col gap-4 mt-6">
        @csrf
        @method('PUT')
        <div class="flex flex-col gap-2">
            <h2 class="text-lg font-semibold">Detail Team</h2>
            <div class="flex flex-col gap-1">
                ...
                <input 
                    ...
                    id="name" 
                    name="name" 
                    class="border border-gray-300 px-4 py-2 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 @error('name') border-red-500 @enderror" 
            ...
            <div class="flex flex-col gap-1">
                ...
                <input 
                    ...
                    min="2024" 
                    max="{{ now()->year }}" 
                    class="border border-gray-300 px-4 py-2 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 @error('year') border-red-500 @enderror" 
        ...
        <div class="flex flex-col gap-2" id="contributors-section">
            ...
            <div id="contributors-container" class="flex flex-col gap-4">
                @php
                    $oldContributors = old('contributors');
                    $teamContributors = $team->contributors->pluck('name')->toArray();
                    $contributors = $oldContributors ?: $teamContributors;
                @endphp
                
                @if($contributors && count($contributors) > 0)
                    @foreach($contributors as $index => $contributor)
                        <div class="contributor-item flex flex-col gap-1" data-contributor="{{ $index + 1 }}">
                            <label for="contributor{{ $index + 1 }}" class="font-semibold">Nama Contributor {{ $index + 1 }}</label>
                            @if($index === 0)
                                <input 
                                    type="text" 
                                    id="contributor{{ $index + 1 }}" 
                                    name="contributors[]" 
                                    class="border border-gray-300 px-4 py-2 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 @error('contributors.' . $index) border-red-500 @enderror" 
                            ...
                            @else
                                <div class="flex items-center gap-3 justify-between">
                                    <input 
                                        ...
                                        id="contributor{{ $index + 1 }}" 
                                        name="contributors[]" 
                                        class="border w-full border-gray-300 px-4 py-2 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 @error('contributors.' . $index) border-red-500 @enderror" 
                                        placeholder="Masukkan nama contributor {{ $index + 1 }}"
                                        value="{{ $contributor }}"
                                    >
                                    <button 
                                        type="button" 
                                        class="delete-contributor text-red-500 hover:text-red-700 hover:bg-red-50 p-2 rounded-lg transition-all duration-200"
                                        data-contributor="{{ $index + 1 }}"
                                        title="Hapus contributor"
                                    >
                                        <svg class="size-5 fill-current" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                @else
                    <div class="contributor-item flex flex-col gap-1" data-contributor="1">
                        <label for="contributor1" class="font-semibold">Nama Contributor 1</label>
                        <input 
                            type="text" 
                            id="contributor1" 
                            name="contributors[]" 
                            class="border border-gray-300 px-4 py-2 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200" 
            ...
            >
                <svg class="size-8 fill-gray-300 mx-auto group-hover:fill-gray-500 transition-all duration-300" viewBox="0 0 26 22" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            window.formHandler = new FormHandler({
                ...
                successMessage: "Team berhasil diperbarui!",
                validators: (ctx) => {
                    let isValid = true;
